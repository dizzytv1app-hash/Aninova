...
# ==================== CONFIG ====================
BOT_TOKEN = "8947239424:AAHQM0bfFsYJLeB5ty_eN7J9iXxLn40VYBI"
ADMIN_ID = 6222096713
...
def episodes_keyboard(season_id, page=0):
    ...
    if episodes:
        buttons.append([InlineKeyboardButton(
            f"📥 Yuklash ({episodes[0]}-{episodes[-1]})", callback_data=f"dlall_{season_id}"
        )])
...
async def episode_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("dlall_"):
        ...
        episodes = get_episodes_list(season_id)
        if not episodes:
            await query.answer("❌ Hali qism yuklanmagan!", show_alert=True)
            return
        ...
        await query.message.reply_text(f"📥 {len(episodes)} ta qism yuklanmoqda, biroz kuting...")
        sent = 0
        for ep_num in episodes:
            ...
            try:
                ...
                sent += 1
                await asyncio.sleep(0.7)  # Telegram flood-limitiga tushib qolmaslik uchun kichik pauza
            except Exception as e:
                logger.warning(
                    "Barchasini yuborishda xato (anime=%s, season=%s, episode=%s): %s",
                    anime_name, season_id, ep_num, e,
                )
        await query.message.reply_text(f"✅ {sent}/{len(episodes)} ta qism yuborildi.")
