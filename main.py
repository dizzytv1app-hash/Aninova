...
BOT_USERNAME = "AniNavo1Bot"  # @ belgisiz, deep-link uchun (t.me/<BOT_USERNAME>?start=kod)
...
def init_db():
    ...
    # Avvalgi versiyada shu cover fasl posteriga yozilgan bo'lishi mumkin; katta anime
    # posteri to'g'ri chiqishi uchun fasl posterlarini asosiy anime posteriga tiklaymiz.
    c.execute("""
        UPDATE seasons
        SET poster_file_id = (
...
async def anime_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔢 Anime kodini yoki nomini yuboring (masalan: 1, 2, 3... yoki \"Naruto\")")
...
async def got_anime_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔤 Original nomini yozing _(yapon/ingliz tilida, bilmasangiz \"-\" yuboring)_:",
        parse_mode="Markdown",
        reply_markup=_cancel_kb()
    )
    return WAIT_ANIME_ORIGINAL
...
async def got_anime_original(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    context.user_data["new_anime_original"] = None if txt == "-" else txt
    await update.message.reply_text(
        "📅 Chiqarilgan yilini yozing:\n_(4 xonali raqam, masalan: 2023, bilmasangiz \"-\" yuboring)_",
        parse_mode="Markdown", reply_markup=_cancel_kb()
    )
    return WAIT_ANIME_YEAR

async def got_anime_year(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    if txt == "-":
        context.user_data["new_anime_year"] = 0
    elif txt.isdigit() and len(txt) == 4:
        context.user_data["new_anime_year"] = int(txt)
    else:
        await update.message.reply_text(
            "⚠️ Faqat 4 xonali yil yuboring (masalan: 2023) yoki \"-\":",
            reply_markup=_cancel_kb()
        )
        return WAIT_ANIME_YEAR
...
async def got_anime_genre_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "gconfirm":
        ...
        await query.message.reply_text(
            "📝 Anime tavsifini (qisqacha syujet) yozing:\n_(bo'lmasa \"-\" yuboring)_",
            parse_mode="Markdown", reply_markup=_cancel_kb()
        )
        return WAIT_ANIME_DESC

async def got_anime_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    context.user_data["new_anime_desc"] = "" if txt == "-" else txt
    await update.message.reply_text(
...
async def got_anime_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.video:
        ...
        poster_type = "photo"
    context.user_data["new_anime_poster_id"] = poster_id
    context.user_data["new_anime_poster_type"] = poster_type

    await update.message.reply_text(
        "⭐ Reytingni yozing _(0 dan 10 gacha, masalan: 8.5, bilmasangiz \"-\" yuboring)_:",
        parse_mode="Markdown", reply_markup=_cancel_kb()
    )
    return WAIT_ANIME_RATING

async def got_anime_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _esc_result = await _check_menu_escape(update, context)
    if _esc_result is not None:
        return _esc_result
    txt = update.message.text.strip()
    if txt == "-":
        context.user_data["new_anime_rating"] = None
    else:
        try:
            rating = float(txt.replace(",", "."))
            if not (0 <= rating <= 10):
                raise ValueError
            context.user_data["new_anime_rating"] = rating
        except ValueError:
            await update.message.reply_text(
                "⚠️ 0 dan 10 gacha raqam yuboring (masalan: 8.5) yoki \"-\":",
                reply_markup=_cancel_kb()
            )
            return WAIT_ANIME_RATING

    buttons = [
        [InlineKeyboardButton(label, callback_data=f"newanimestatus_{key}")]
    ...
    return WAIT_ANIME_STATUS

async def got_anime_status_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    status_key = query.data.split("_", 1)[1]
    d = context.user_data
    poster_id = d.get("new_anime_poster_id")
    poster_type = d.get("new_anime_poster_type", "photo")
    ...
    try:
        ...
        await query.message.reply_text(
            ...
            reply_markup=admin_menu_keyboard(query.from_user.id)
        )
    except Exception as e:
...
async def quick_episode_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"🖼 *{_esc_md(anime[2])}* uchun qismlar posteri sifatida ishlatiladigan rasm yuboring:",
        parse_mode="Markdown"
    )
...
async def got_episode_poster(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Anime kodi tanlangach, uning barcha qismlarida ishlatiladigan posterini qabul qiladi."""
    ...
    context.user_data["ep_poster_type"] = poster_type
    set_anime_episode_cover(code, poster_id)
    await update.message.reply_text(
        f"✅ *{_esc_md(anime_name)}* qismlari uchun poster saqlandi.\n\nEndi qaysi faslga qism qo'shmoqchisiz?",
        parse_mode="Markdown"
    )
...
async def _create_new_season_with_episode_poster(message, context, poster_id, poster_type):
    """Yangi faslni anime asosiy posterida yaratadi; qism cover'i alohida saqlanadi."""
    code = context.user_data.get("ep_anime_code")
    total_ep = context.user_data.get("new_season_total", 0)
    ep_label = context.user_data.get("new_season_total_label")
    main_poster = get_anime_main_poster(code) if code else None
    if not code or not poster_id or not main_poster or not main_poster[0]:
        await message.reply_text("⚠️ Anime, asosiy poster yoki qism posteri topilmadi. Jarayonni qayta boshlang.")
        return ConversationHandler.END
    main_poster_id, main_poster_type = main_poster
    seasons = get_seasons(code)
    next_season_num = (max(s[1] for s in seasons) + 1) if seasons else 1
    season_id = add_season(code, next_season_num, main_poster_id, total_ep, poster_type=main_poster_type, total_episodes_label=ep_label)
    context.user_data["ep_season_id"] = season_id
...
async def got_newseason_episodes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    poster_id = context.user_data.get("ep_poster_id")
    poster_type = context.user_data.get("ep_poster_type", "photo")
    if poster_id:
        return await _create_new_season_with_episode_poster(update.message, context, poster_id, poster_type)
    # Faqat eski yoki uzilib qolgan jarayonlar uchun zaxira yo‘l.
    await update.message.reply_text("🖼 Qism posteri topilmadi. Rasm yoki video yuboring:")
...
async def backup_db_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with backup_conn:
            conn.backup(backup_conn)  # WAL rejimida ham xavfsiz, izchil nusxa oladi
        backup_conn.close()
        with open(tmp_path, "rb") as f:
            await update.message.reply_document(
                document=f,
                filename=filename,
                caption=f"💾 Zaxira nusxa tayyor.\n🕐 {datetime.now().strftime('%Y-%m-%d %H:%M')}"
            )
    except Exception as e:
...
async def got_restore_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc or not doc.file_name.lower().endswith(".db"):
        await update.message.reply_text("❌ Faqat *.db* fayl yuboring!", parse_mode="Markdown")
        return
...
async def restore_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.message.edit_text("✅ Backup muvaffaqiyatli tiklandi.")
    except Exception:
...
def main():
    ...
    add_anime_conv = ConversationHandler(
        ...
        states={
            ...
            WAIT_ANIME_ORIGINAL: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_anime_original)],
            WAIT_ANIME_YEAR: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_anime_year)],
            WAIT_ANIME_GENRE: [CallbackQueryHandler(got_anime_genre_callback, pattern="^(gsel_|gconfirm)")],
            WAIT_ANIME_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_anime_desc)],
            ...
            WAIT_ANIME_RATING: [MessageHandler(filters.TEXT & ~filters.COMMAND, got_anime_rating)],
